...
public:
	...
	*/
	...
	// Table that returs, for a given lead UInt8, the total number of UInt8s
	// in the UTF-8 sequence.
	static const int utf8UInt8Table[256];
...
protected:
	...
	*/
	...
	// The length can be from 0 to 4 UInt8s.
	inline static const char* GetChar( const char* p, char* _value, int* length, TiXmlEncoding encoding )
	{
		assert( p );
		if ( encoding == TIXML_ENCODING_UTF8 )
		{
			*length = utf8UInt8Table[ *((unsigned char*)p) ];
	...
	// to Engilish UInt16s: StringEqual( p, "version", true ) is fine.
	...
	static int IsAlpha( unsigned char anyUInt8, TiXmlEncoding encoding );
	static int IsAlphaNum( unsigned char anyUInt8, TiXmlEncoding encoding );
